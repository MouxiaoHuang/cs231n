...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        pass
        """f = scores - np.max(scores, axis=1, keepdims=True) # scores减去scores每一行的最大值。axis=1按行计算，减去最大值是为了避免scores过大而使softmax指数爆炸
      #  f = np.exp(f) / np.exp(f).sum(axis=1, keepdims=True)                                                         #### ??????????为什么不归一化呢？
        ...
        total_loss = -np.log(prob)  # [N, 1]            #  此处用的是默认公式，没用等价公式
        ...
        dhidden_layer_raw = dhidden_layer * (hidden_layer>0)
        grads['W1'] = X.T.dot(dhidden_layer_raw) + 2 * reg * W1
        grads['b1'] = np.sum(dhidden_layer_raw, axis=0)
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in range(num_iters):
            ...
            pass
            index = np.random.choice(num_train, batch_size, replace=True)   #  随机采样
            X_batch = X[index]
            y_batch = y[index]
            ...
            pass
            self.params['W2'] -= learning_rate * grads['W2']  # 更新参数
    ...
    def predict(self, X):
        ...
        pass
        W1 = self.params['W1']  # 取得训练后的最新的网络参数
        b1 = self.params['b1']
        W2 = self.params['W2']
        b2 = self.params['b2']
